...
<form action="{{route('store_product')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group col-md-6 offset-3 mt-5">
        <input type="text" class="form-control mt-2" placeholder="name" name="name" value="{{old('name')}}">
        @error('name')
            <h5 class="text-danger">{{$message}}</h5>
        @enderror
        <input type="text" class="form-control mt-2" placeholder="price" name="price" value="{{old('price')}}">
        @error('price')
        <h5 class="text-danger">{{$message}}</h5>
        @enderror
